...
from src_both.lib.consts import DATASET_SHIFTS, TRAIN_METHODS
from src.lib.utils import calibration_temp, negentropy, load_shift_data
...
if __name__ == '__main__':
    ...
    for dataset in DATASET_SHIFTS.keys():
        for shift, method in itertools.product(DATASET_SHIFTS[dataset], TRAIN_METHODS):
            ...
            try:
                (source_feats, source_labels), (target_feats, target_labels), (h1_source_out, h1_target_out) = load_shift_data(args.feats_dir, dataset, shift, method, device)
